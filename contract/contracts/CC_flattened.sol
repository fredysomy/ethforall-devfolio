...
contract CCNFT is ERC721URIStorage {
  ...
  function makeAnEpicNFT(
    ...
    string memory Svg = string(abi.encodePacked(svgPartOne, donatorAddress, svgPartTwo, receiverName, svgPartThree, serviceName, svgPartFour, ticketNumber, svgPartFive));
...
contract CC is CCNFT{
    ...
    constructor() {
        owner = msg.sender;
        usdcToken = ERC20(0x64c61eFac6383d0F8A4cff6aDE93c474ece7AD44);
     }
    ...
    mapping(address => uint256[]) public donatorsToServices;
    ...
    function registerNewService(uint256 amount, uint256 duration, uint256 maxNumberOfTickets, string memory description,address recvaddress, string memory serviceName) public {
            
            uint256 startTime = block.timestamp;
            uint256 endTime = startTime + duration;
            services[serviceCount] = Service(serviceCount, msg.sender, amount, startTime, endTime, duration, maxNumberOfTickets, description, recvaddress, serviceName);
            _depositStake(amount/4, msg.sender, serviceCount);
            serviceEnabled[serviceCount] = true; 
            serviceCount += 1;
    }

    function _depositStake(uint _amount, address _provider, uint256 _serviceCount) public{
    ...
    function _withdrawStake(uint serviceID) public returns (bool){
        require(stakes[serviceID].amount > 0, "No stake to withdraw");
        require(stakes[serviceID].provider == msg.sender, "YOU ARE NOT AUTHORISED TO WITHDRAW THIS STAKE");

        uint256 amount = stakes[serviceID].amount;

        stakes[serviceID].amount = 0;
        serviceEnabled[serviceCount] = false; 
        usdcToken.transfer(msg.sender, amount);
        return true;
    }

    function _reEnableService(uint256 serviceID) public {
        ...
        if(stakes[serviceID].amount == 0){
            _depositStake(service.amount / 4, msg.sender, serviceID);
            serviceEnabled[serviceCount] = true; 
        }
        else{
            serviceEnabled[serviceCount] = true;
    ...
    function registerNewR(uint rid, address raddress, string memory rName) public restrictToOwner(){ //need to add a onlyOwner modifier
            receivers[receivercount] = Receiver(rid, raddress,rName);
            receiversByAddress[raddress] = Receiver(rid,raddress, rName);
    ...
    function donateToService(uint serviceID) public{
                ...
                require(serviceEnabled[serviceID] != true, "Service is disabled"); // service should not be disabled
                ...
                usdcToken.transferFrom(msg.sender, service.recvaddress, service.amount);
                        ...
                        donatorsToServices[msg.sender].push(serviceID);
                        donatorsToDonations[msg.sender][serviceID].push(tokenID);    
                        console.log("An NFT has been minted to %s", msg.sender);
    ...
    function getServiceTokenIDsDonatedByUser(uint256 serviceID) view public returns(uint256[] memory) {
            return donatorsToDonations[msg.sender][serviceID];
    }
